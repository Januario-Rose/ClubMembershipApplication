        ...
        private bool ValidField(int fieldIndex, string fieldValue, string[] fieldArray, out string fieldInvalidMessage)
        {
            ...
            switch (userRegistrationField)
            {
                ...
                case FieldConstants.UserRegistrationField.PhoneNumber:
                    fieldInvalidMessage = (!_requiredValidDel(fieldValue)) ? $"You must enter a value for field:{Enum.GetName(typeof(FieldConstants.UserRegistrationField), userRegistrationField)}{Environment.NewLine}" : "";
                    fieldInvalidMessage = (fieldInvalidMessage == "" && !_patternMatchValidDel(fieldValue, CommonRegularExpressionValidationPatterns.SA_PhoneNumber_RegEx_Pattern)) ? $"You did not enter a valid SA phone number{Environment.NewLine}" : fieldInvalidMessage;
                    break;
                ...
                case FieldConstants.UserRegistrationField.PostalCode:
                    fieldInvalidMessage = (!_requiredValidDel(fieldValue)) ? $"You must enter a value for field:{Enum.GetName(typeof(FieldConstants.UserRegistrationField), userRegistrationField)}{Environment.NewLine}" : "";
                    fieldInvalidMessage = (fieldInvalidMessage == "" && !_patternMatchValidDel(fieldValue, CommonRegularExpressionValidationPatterns.SA_Postal_Code_RegEx_Pattern)) ? $"You did not enter a valid SA postal code{Environment.NewLine}" : fieldInvalidMessage;
                    break;
                default:
